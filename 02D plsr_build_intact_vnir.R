...
## that only use 400-1000 nm

sol_intact<-plsr(meta(intact_spec_agg_train)$solubles~as.matrix(intact_spec_agg_train[,400:1000]),
                 ncomp=30,method = "oscorespls",validation="CV",segments=10)
hemi_intact<-plsr(meta(intact_spec_agg_train)$hemicellulose~as.matrix(intact_spec_agg_train[,400:1000]),
                  ncomp=30,method = "oscorespls",validation="CV",segments=10)
recalc_intact<-plsr(meta(intact_spec_agg_train)$recalcitrant~as.matrix(intact_spec_agg_train[,400:1000]),
                    ncomp=30,method = "oscorespls",validation="CV",segments=10)
Nmass_intact<-plsr(meta(intact_spec_agg_train)$Nmass~as.matrix(intact_spec_agg_train[,400:1000]),
                   ncomp=30,method = "oscorespls",validation="CV",segments=10)
Cmass_intact<-plsr(meta(intact_spec_agg_train)$Cmass~as.matrix(intact_spec_agg_train[,400:1000]),
                   ncomp=30,method = "oscorespls",validation="CV",segments=10)
LMA_intact<-plsr(meta(intact_spec_agg_train)$LMA~as.matrix(intact_spec_agg_train[,400:1000]),
                 ncomp=30,method = "oscorespls",validation="CV",segments=10)
Narea_intact<-plsr(meta(intact_spec_agg_train)$Narea~as.matrix(intact_spec_agg_train[,400:1000]),
                   ncomp=30,method = "oscorespls",validation="CV",segments=10)
Carea_intact<-plsr(meta(intact_spec_agg_train)$Carea~as.matrix(intact_spec_agg_train[,400:1000]),
                   ncomp=30,method = "oscorespls",validation="CV",segments=10)
...
VIP_intact<-data.frame(sol=VIP(sol_intact)[ncomp_sol_intact,],
                       ...
                       wavelength=400:1000)
saveRDS(VIP_intact,"SavedResults/VIP_intact_vnir.rds")
...
for(i in 1:nreps){
  ...
  sol_intact_jack<-plsr(meta(calib_jack)$solubles~as.matrix(calib_jack[,400:1000]),
                        ncomp=30,method = "oscorespls",validation="none")
  hemi_intact_jack<-plsr(meta(calib_jack)$hemicellulose~as.matrix(calib_jack[,400:1000]),
                         ncomp=30,method = "oscorespls",validation="none")
  recalc_intact_jack<-plsr(meta(calib_jack)$recalcitrant~as.matrix(calib_jack[,400:1000]),
                           ncomp=30,method = "oscorespls",validation="none")
  Cmass_intact_jack<-plsr(meta(calib_jack)$Cmass~as.matrix(calib_jack[,400:1000]),
                          ncomp=30,method = "oscorespls",validation="none")
  Nmass_intact_jack<-plsr(meta(calib_jack)$Nmass~as.matrix(calib_jack[,400:1000]),
                          ncomp=30,method = "oscorespls",validation="none")
  Carea_intact_jack<-plsr(meta(calib_jack)$Carea~as.matrix(calib_jack[,400:1000]),
                          ncomp=30,method = "oscorespls",validation="none")
  Narea_intact_jack<-plsr(meta(calib_jack)$Narea~as.matrix(calib_jack[,400:1000]),
                          ncomp=30,method = "oscorespls",validation="none")
  LMA_intact_jack<-plsr(meta(calib_jack)$LMA~as.matrix(calib_jack[,400:1000]),
                        ncomp=30,method = "oscorespls",validation="none")
  
  sol_jack_val_pred<-as.vector(predict(sol_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_sol_intact)[,,1])
  ...
  hemi_jack_val_pred<-as.vector(predict(hemi_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_hemi_intact)[,,1])
  ...
  recalc_jack_val_pred<-as.vector(predict(recalc_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_recalc_intact)[,,1])
  ...
  Cmass_jack_val_pred<-as.vector(predict(Cmass_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_Cmass_intact)[,,1])
  ...
  Nmass_jack_val_pred<-as.vector(predict(Nmass_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_Nmass_intact)[,,1])
  ...
  Carea_jack_val_pred<-as.vector(predict(Carea_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_Carea_intact)[,,1])
  ...
  Narea_jack_val_pred<-as.vector(predict(Narea_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_Narea_intact)[,,1])
  ...
  LMA_jack_val_pred<-as.vector(predict(LMA_intact_jack,newdata=as.matrix(val_jack[,400:1000]),ncomp=ncomp_LMA_intact)[,,1])
...
sol_jack_pred<-apply.coefs(sol_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
hemi_jack_pred<-apply.coefs(hemi_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
recalc_jack_pred<-apply.coefs(recalc_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
Cmass_jack_pred<-apply.coefs(Cmass_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
Nmass_jack_pred<-apply.coefs(Nmass_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
Carea_jack_pred<-apply.coefs(Carea_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
Narea_jack_pred<-apply.coefs(Narea_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
LMA_jack_pred<-apply.coefs(LMA_jack_coefs,as.matrix(intact_spec_agg_test[,400:1000]))
...
saveRDS(intact_jack_coef_list,"SavedResults/intact_jack_coefs_list_vnir.rds")
...
intact_jack_df_list<-list(LMA=LMA_jack_df,
                          ...
                          Carea=Carea_jack_df,
                          Narea=Narea_jack_df)
saveRDS(intact_jack_df_list,"SavedResults/intact_jack_df_list_vnir.rds")
...
VIP_intact_vnir_long<-melt(VIP_intact,id.vars = "wavelength")
levels(VIP_intact_vnir_long$variable)<-c("sol","hemi","recalc","LMA",
                                         "Cmass","Nmass","Carea","Narea")

plot_left<-c("sol","hemi","recalc","LMA")

VIP_intact_vnir_long$side<-ifelse(VIP_intact_vnir_long$variable %in% plot_left,
                                  "left","right")

VIP_intact_vnir_plot_facet<-ggplot(VIP_intact_vnir_long,
  ...
  scale_x_continuous(expand = c(0, 0),limits=c(370,1030))+
  ylim(c(0,3))

pdf("Manuscript/FigS3.pdf",height=5,width=10)
VIP_intact_vnir_plot_facet
dev.off()
